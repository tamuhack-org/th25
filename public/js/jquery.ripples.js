+function ($) {
	...
	var Ripples = function (el, options) {
		...
		if (!this.$img[0].complete || !this.$img[0].naturalWidth) {
			...
			this.$img.on('load', function() {
				...
				canvas = setupCanvas(that, that.$img[0].naturalWidth, that.$img[0].naturalHeight)
				if (!canvas) {
				...
				console.log('Image dimensions:', that.$img[0].naturalWidth, that.$img[0].naturalHeight);
	...
	Ripples.prototype = {
		...
		render: function() {
			...
			// Validate uniforms
			if (!this.renderProgram.uniforms || 
			...
			if (!locations.topLeft || !locations.bottomRight || !locations.containerRatio) {
		...
		computeTextureBoundaries: function() {
			...
			var backgroundX = imageOffset.left + (containerWidth - backgroundWidth * scale) / 2;
			var backgroundY = imageOffset.top + (containerHeight - backgroundHeight * scale) / 2;
		...
		initShaders: function() {
			...
			try {
				...
				var deltaLocation = gl.getUniformLocation(this.updateProgram[0].id, 'delta');
				if (deltaLocation) {
				...
				deltaLocation = gl.getUniformLocation(this.updateProgram[1].id, 'delta');
				if (deltaLocation) {
					gl.uniform2fv(deltaLocation, this.textureDelta);
				}
				...
				gl.useProgram(this.renderProgram.id);
				var perturbanceLocation = gl.getUniformLocation(this.renderProgram.id, 'perturbance');
	...
	$.fn.ripples = function(option) {
		return this.each(function() {
			...
			var options = $.extend({}, Ripples.DEFAULTS, $this.data(), 
